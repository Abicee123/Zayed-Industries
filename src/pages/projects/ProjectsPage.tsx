...
export default function ProjectsPage() {
  ...
  const [filterCompanyId, setFilterCompanyId] = useState<string>("all"); // NEW: Global Admin Company Filter
  ...
  // NEW: Refined visible projects logic with Subsidiary Filter
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-8 animate-in fade-in duration-700 pb-8 print:p-0 print:m-0">
      ...
      <div className="bg-white p-2 rounded-2xl border border-slate-100 shadow-sm flex flex-col sm:flex-row gap-2 print:hidden">
        ...
        <div className="sm:w-48 shrink-0 border-t sm:border-t-0 sm:border-l border-slate-100 pt-2 sm:pt-0 sm:pl-2">
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full h-11 rounded-xl bg-slate-50 border-none px-4 text-sm font-bold text-slate-700 outline-none cursor-pointer appearance-none"
             style={{ backgroundImage: `url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%2394a3b8%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpolyline%20points%3D%226%209%2012%2015%2018%209%22%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 12px center', backgroundSize: '16px' }}
          >
            ...
            {visibleProjects.map(project => {
              ...
              return (
                <div key={project.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:border-blue-200 transition-all flex flex-col relative overflow-hidden group">
                  ...
                  {dueStatus && (
                    <div className={`ml-1.5 px-4 py-2 border-b flex items-center justify-center gap-1.5 text-[9px] font-bold uppercase tracking-widest ${dueStatus.style}`}>
                      <dueStatus.icon className="h-3 w-3" /> {dueStatus.label}
                    </div>
                  )}
                  ...
                  <div className="ml-1.5 p-7 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider mt-1 mb-4 truncate">
                        ...
                        {project.internal_company_id && ` • Internal: ${companies.find(c => c.id === project.internal_company_id)?.name}`}
                      </p>
                    ...
                    <div className="w-full md:w-44 flex flex-col gap-2.5 md:border-l md:border-slate-100 md:pl-6">
                       {(role === 'admin' || role === 'head') && (
                         <div className="flex justify-between items-center bg-slate-50 px-3.5 py-1.5 rounded-xl border border-slate-100">
                           ...
                           <span className="text-xs font-bold text-emerald-600">₹{(project.expected_amount || 0).toLocaleString()}</span>
                         </div>
                       )}
      ...
      <AnimatePresence>
        {isModalOpen && !isPrintingPayslip && (
          <div className="absolute inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-sm print:hidden">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden border border-slate-100">
              ...
              <div className="px-8 pt-7 border-b border-slate-100 bg-[#FAFCFF] shrink-0">
                ...
                <div className="flex gap-8 overflow-x-auto [&::-webkit-scrollbar]:hidden">
                  <button onClick={() => setModalTab('details')} className={`pb-3 text-[11px] font-bold uppercase tracking-wider transition-all border-b-2 whitespace-nowrap ${modalTab === 'details' ? 'border-blue-900 text-blue-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>1. Details & Tasks</button>
                  <button onClick={() => setModalTab('progress')} disabled={!selectedProject} className={`pb-3 text-[11px] font-bold uppercase tracking-wider transition-all border-b-2 whitespace-nowrap ${!selectedProject ? 'opacity-30 cursor-not-allowed' : modalTab === 'progress' ? 'border-blue-900 text-blue-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>2. Reports Timeline</button>
                  <button onClick={() => setModalTab('finance')} disabled={!selectedProject} className={`pb-3 text-[11px] font-bold uppercase tracking-wider transition-all border-b-2 whitespace-nowrap ${!selectedProject ? 'opacity-30 cursor-not-allowed' : modalTab === 'finance' ? 'border-blue-900 text-blue-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>3. Financial Allocations</button>
                </div>
              </div>
                  ...
                  <div className="flex-1 space-y-6">
                    ...
                    <div className="bg-slate-50 border border-slate-100 rounded-3xl p-6 space-y-6">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                           ...
                           <p className="text-[11px] text-slate-500 mt-0.5 font-medium">Link project to a customer or internal node.</p>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">Issuing Subsidiary</label>
                        ...
                        <div>
                          {customerType === 'existing' && (
                            <><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">Customer / Client *</label>
                          ...
                          {customerType === 'new' && (
                            <><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">New Client *</label>
                            <div className="grid grid-cols-2 gap-3">
                              <input type="text" placeholder="Client Name *" value={newCustomer.name} onChange={e => setNewCustomer({...newCustomer, name: e.target.value})} disabled={role === 'user'} className="w-full h-12 rounded-xl border border-slate-200 bg-white px-4 text-[13px] font-medium outline-none focus:border-blue-500 shadow-sm disabled:bg-slate-100" />
                              <input type="text" placeholder="Phone (Optional)" value={newCustomer.phone} onChange={e => setNewCustomer({...newCustomer, phone: e.target.value})} disabled={role === 'user'} className="w-full h-12 rounded-xl border border-slate-200 bg-white px-4 text-[13px] font-medium outline-none focus:border-blue-500 shadow-sm disabled:bg-slate-100" />
                            </div></>
                          )}
                          {customerType === 'in_house' && (
                            <><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">Internal Company Target *</label>
                            <select value={formData.internal_company_id} onChange={(e) => setFormData({...formData, internal_company_id: e.target.value, customer_id: ""})} disabled={role === 'user'} className="w-full h-12 rounded-xl border border-slate-200 bg-blue-50/50 px-4 text-[13px] font-bold text-blue-900 outline-none focus:border-blue-500 shadow-sm disabled:bg-slate-100 disabled:text-slate-400">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                      <div className="md:col-span-3">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1 whitespace-nowrap truncate">Project Name</label>
                        <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} disabled={role === 'user'} className="w-full h-12 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 shadow-sm disabled:bg-slate-50" />
                      </div>
                      <div>
                        <label className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest block mb-2 px-1 whitespace-nowrap truncate">Expected Value (₹)</label>
                        <input type="number" value={formData.expected_amount} onChange={(e) => setFormData({...formData, expected_amount: parseFloat(e.target.value) || 0})} disabled={role === 'user'} placeholder="For Auto-Invoice" className="w-full h-12 rounded-xl border border-emerald-200 bg-emerald-50 px-4 text-sm font-bold text-emerald-700 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 shadow-sm disabled:bg-slate-50 disabled:text-slate-400 disabled:border-slate-200" />
                      </div>
                    </div>

                    <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">Description</label><textarea value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} disabled={role === 'user'} className="w-full h-24 rounded-xl border border-slate-200 bg-white p-4 text-sm font-medium outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 shadow-sm resize-none disabled:bg-slate-50" /></div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                      <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">Priority</label><select value={formData.priority} onChange={(e) => setFormData({...formData, priority: e.target.value})} disabled={role === 'user'} className="w-full h-12 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium outline-none shadow-sm cursor-pointer disabled:bg-slate-50"><option value="Low">Low</option><option value="Medium">Medium</option><option value="High">High</option></select></div>
                      <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">Approval Date</label><input type="date" value={formData.approval_date} onChange={(e) => setFormData({...formData, approval_date: e.target.value})} disabled={role === 'user'} className="w-full h-12 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium outline-none shadow-sm disabled:bg-slate-50" /></div>
                      <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">Due Date</label><input type="date" value={formData.due_date} onChange={(e) => setFormData({...formData, due_date: e.target.value})} disabled={role === 'user'} className="w-full h-12 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium outline-none shadow-sm disabled:bg-slate-50" /></div>
                    </div>

                    {(role === 'admin' || role === 'head') && (
                      <div className="pt-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-3 px-1">Assign Team Members</label>
                  ...
                  <div className="lg:w-[380px] shrink-0 flex flex-col lg:border-l lg:border-t-0 border-t border-slate-100 lg:pl-8 pt-6 lg:pt-0">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 pr-2 max-h-[350px] lg:max-h-[500px] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full">
                      {displayTasks.length === 0 ? (
                        <div className="h-40 flex flex-col items-center justify-center text-slate-400 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200"><p className="text-[11px] font-bold uppercase tracking-wider">No tasks added</p></div>
                      ) : (
                    ...
                    {(role === 'admin' || role === 'head') && (
                      <div className="mt-6 pt-6 border-t border-slate-100 space-y-3">
                         <input type="text" placeholder="New task title..." value={newTaskTitle} onChange={(e) => setNewTaskTitle(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddTask()} className="w-full h-12 rounded-xl border border-slate-200 px-4 text-[13px] font-medium outline-none focus:border-blue-500 shadow-sm" />
                         <div className="flex gap-2">
                           ...
                           <button onClick={handleAddTask} className="h-12 w-12 bg-gradient-to-r from-blue-900 to-indigo-800 text-white rounded-xl flex items-center justify-center font-bold shadow-md hover:shadow-lg transition-all"><Plus className="h-5 w-5" /></button>
                         </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center">
                  <div className="w-full max-w-3xl flex flex-col space-y-6">
                     <div className="flex items-center justify-between mb-2 border-b border-slate-100 pb-4">
                       <h3 className="text-base font-bold text-slate-900 uppercase tracking-wider">Reports Timeline</h3>
                       <Star className="h-6 w-6 text-amber-400 fill-amber-400" />
                     </div>
                     
                     {role === 'user' ? (
                       <div className="flex flex-col bg-white border border-slate-100 rounded-3xl p-8 shadow-sm h-full min-h-[400px]">
                         
                         <div className="flex-1 overflow-y-auto mb-6 pr-4 space-y-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full">
                           <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest sticky top-0 bg-white pb-2 border-b border-slate-100 mb-4 z-10">Previous Updates</h4>
                           {reports.find(r => r.project_id === selectedProject?.id && r.employee_id === employeeId)?.report_text ? (
                             <pre className="text-[13px] text-slate-700 whitespace-pre-wrap font-sans leading-relaxed bg-slate-50 p-5 rounded-2xl border border-slate-100">
                               {reports.find(r => r.project_id === selectedProject?.id && r.employee_id === employeeId)?.report_text}
                             </pre>
                           ) : (
                             <p className="text-sm italic text-slate-400">No updates submitted yet.</p>
                           )}
                         </div>

                         <div className="border-t border-slate-100 pt-6">
                           <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Add New Log Entry</p>
                           <textarea value={myReportText} onChange={e => setMyReportText(e.target.value)} placeholder="Type your latest progress update here..." className="w-full h-24 rounded-2xl border border-slate-200 bg-white p-5 text-sm font-medium outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all resize-none mb-4 shadow-sm" />
                           <div className="flex justify-between items-center">
                             <div className="flex items-center gap-2.5 text-amber-500 font-bold text-sm bg-amber-50 px-3 py-1.5 rounded-lg border border-amber-200"><Star className="h-4 w-4 fill-amber-500" /> Grade: {reports.find(r => r.project_id === selectedProject?.id && r.employee_id === employeeId)?.rating || '--'} / 10</div>
                             <button onClick={handleSaveMyReport} className="bg-gradient-to-r from-blue-900 to-indigo-800 text-white rounded-xl h-11 px-8 text-sm font-bold shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all">Append Update</button>
                           </div>
                         </div>
                       </div>
                     ) : (
                       <div className="space-y-5">
                         {(formData.assignee_ids || []).length === 0 && <p className="text-sm text-center text-slate-400 italic py-10">No team members assigned.</p>}
                         {(formData.assignee_ids || []).map(empId => {
                           ...
                           return (
                             <div key={empId} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm">
                               ...
                               <div className="bg-slate-50 rounded-2xl p-5 min-h-[80px] max-h-[300px] overflow-y-auto">
                                 {empReport?.report_text ? <pre className="text-[13px] text-slate-700 leading-relaxed font-sans whitespace-pre-wrap">{empReport.report_text}</pre> : <p className="text-sm text-slate-400 italic">No timeline entries.</p>}
                               </div>
                             </div>
                           )
                         })}
              ...
              {modalTab === 'finance' && (
                <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center">
                  ...
                  {role === 'user' ? (
                     <div className="w-full max-w-2xl bg-white border border-slate-100 rounded-3xl p-10 shadow-sm flex flex-col items-center">
                        <Wallet className="h-16 w-16 text-emerald-500 mb-6" />
                        <h2 className="text-3xl font-black text-slate-900 tracking-tight mb-2">My Financial Ledger</h2>
                        <p className="text-sm font-medium text-slate-500 mb-10 text-center">Your compensation breakdown and payment history for {selectedProject.name}.</p>
                        ...
                        <div className="w-full grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                           <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 text-center">
                              ...
                              <p className="text-2xl font-black text-emerald-600">₹{userTotalAllocated.toLocaleString()}</p>
                           </div>
                           <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 text-center">
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Payments Rec'd</p>
                              <p className="text-2xl font-black text-slate-800">₹{userTotalEarned.toLocaleString()}</p>
                           </div>
                           <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 text-center">
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Balance Due</p>
                              <p className="text-2xl font-black text-rose-500">₹{userBalanceDue.toLocaleString()}</p>
                           </div>
                        </div>

                        <button onClick={handlePrintPayslip} disabled={isPrintingPayslip} className="bg-gradient-to-r from-blue-900 to-indigo-800 text-white rounded-xl h-14 px-10 text-sm font-bold shadow-xl shadow-blue-900/20 hover:shadow-2xl hover:-translate-y-1 transition-all flex items-center gap-2">
                           <Download className="h-5 w-5" /> Download PDF Payslip
                        </button>
                     </div>
                  ) : (
                     <div className="w-full max-w-5xl space-y-8">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                          <div>
                            <h3 className="text-xl font-bold text-slate-900">Project Compensation & Payouts</h3>
                            <p className="text-sm font-medium text-slate-500 mt-1">Assign funds and issue payments from this project's revenue.</p>
                          </div>
                        ...
                        <div className="bg-white border border-slate-100 shadow-sm rounded-3xl overflow-hidden">
                           ...
                           {(formData.assignee_ids || []).length === 0 && <p className="text-sm text-center text-slate-400 italic py-8">No team members assigned.</p>}
                           ...
                           <div className="divide-y divide-slate-50">
                              {(formData.assignee_ids || []).map(empId => {
                                 ...
                                 return (
                                   <div key={empId} className="grid grid-cols-12 gap-4 items-center px-6 py-4 hover:bg-blue-50/30 transition-colors">
                                      <div className="col-span-3 flex items-center gap-3">
                                        <div className="h-8 w-8 rounded-full bg-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-600 overflow-hidden shadow-sm">{emp?.profile_image_url ? <img src={emp.profile_image_url} alt="" className="h-full w-full object-cover" /> : (emp?.name || 'U').charAt(0).toUpperCase()}</div>
                                        <span className="text-sm font-bold text-slate-900 truncate">{emp?.name}</span>
                                      </div>
                                      <div className="col-span-2">
                                         <input type="number" value={alloc.allocated} onChange={e => setAllocationsForm({...allocationsForm, [empId]: {...alloc, allocated: parseFloat(e.target.value)||0}})} className="w-full h-10 border border-slate-200 rounded-xl px-3 font-bold text-sm outline-none focus:border-blue-500 transition-all" />
                                      </div>
                                      <div className="col-span-2">
                                         <input type="number" value={alloc.incentive} onChange={e => setAllocationsForm({...allocationsForm, [empId]: {...alloc, incentive: parseFloat(e.target.value)||0}})} className="w-full h-10 border border-emerald-200 bg-emerald-50 text-emerald-700 rounded-xl px-3 font-bold text-sm outline-none focus:border-emerald-500 transition-all" />
                                      </div>
                                      <div className="col-span-2 text-sm font-bold text-emerald-600">
                                         ₹{empPaid.toLocaleString()}
                                      </div>
                                      <div className="col-span-3 text-right text-base font-black text-slate-900">
                           ...
                           <div className="bg-slate-50 px-6 py-4 border-t border-slate-100 flex justify-end">
                              <button onClick={handleSaveAllocations} disabled={isSaving} className="bg-white border border-slate-200 text-slate-800 hover:text-blue-600 hover:border-blue-200 rounded-xl h-10 px-6 text-xs font-bold shadow-sm transition-all">Save Allocations</button>
                           </div>
                        </div>

                        <div className="bg-emerald-50/50 border border-emerald-100 rounded-3xl p-8">
                           <h4 className="text-sm font-bold text-emerald-800 uppercase tracking-widest mb-6">Issue Payout / Advance</h4>
                           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
                              <div className="lg:col-span-1">
                                <label className="text-[10px] font-bold text-emerald-600 uppercase block mb-1.5 px-1">Employee</label>
                                <select value={paymentForm.employee_id} onChange={e => setPaymentForm({...paymentForm, employee_id: e.target.value})} className="w-full h-11 rounded-xl border border-emerald-200 bg-white px-3 text-sm font-bold text-slate-800 outline-none focus:border-emerald-500 shadow-sm cursor-pointer"><option value="">-- Select --</option>{(formData.assignee_ids || []).map(id => <option key={id} value={id}>{getAvatar(id)?.name}</option>)}</select>
                              </div>
                              <div className="lg:col-span-1">
                                <label className="text-[10px] font-bold text-emerald-600 uppercase block mb-1.5 px-1">Amount (₹)</label>
                                <input type="number" value={paymentForm.amount} onChange={e => setPaymentForm({...paymentForm, amount: parseFloat(e.target.value)||0})} className="w-full h-11 rounded-xl border border-emerald-200 bg-white px-3 text-sm font-black text-emerald-700 outline-none focus:border-emerald-500 shadow-sm" />
                              </div>
                              <div className="lg:col-span-1">
                                <label className="text-[10px] font-bold text-emerald-600 uppercase block mb-1.5 px-1">Type</label>
                                <select value={paymentForm.payment_type} onChange={e => setPaymentForm({...paymentForm, payment_type: e.target.value})} className="w-full h-11 rounded-xl border border-emerald-200 bg-white px-3 text-sm font-bold text-slate-800 outline-none focus:border-emerald-500 shadow-sm cursor-pointer"><option>Advance</option><option>Final Payout</option><option>Incentive / Bonus</option></select>
                              </div>
                              <div className="lg:col-span-2">
                                <label className="text-[10px] font-bold text-emerald-600 uppercase block mb-1.5 px-1">Notes (Optional)</label>
                                <div className="flex gap-2">
                                  <input type="text" placeholder="Ref or details..." value={paymentForm.notes} onChange={e => setPaymentForm({...paymentForm, notes: e.target.value})} className="flex-1 h-11 rounded-xl border border-emerald-200 bg-white px-3 text-sm font-medium outline-none focus:border-emerald-500 shadow-sm" />
                                  <button onClick={handleRecordProjectPayment} disabled={isSaving} className="h-11 px-6 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-xs font-bold rounded-xl shadow-md hover:shadow-lg transition-all shrink-0">Transfer</button>
                                </div>
                              </div>
                           </div>

                           <div className="mt-8 border-t border-emerald-100 pt-8">
                             <h4 className="text-sm font-bold text-emerald-800 uppercase tracking-widest mb-6">Recorded Payouts History</h4>
                             <div className="space-y-3">
                                {salaryPayments.filter(sp => sp.project_id === selectedProject.id).length === 0 ? (
                                  <p className="text-sm italic text-emerald-600/70">No payments recorded for this project.</p>
                                ) : salaryPayments.filter(sp => sp.project_id === selectedProject.id).map(p => (
                                  <div key={p.id} className="flex justify-between items-center bg-white p-4 rounded-xl border border-emerald-100 shadow-sm">
                                     <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                          <p className="text-sm font-bold text-slate-900">{getAvatar(p.employee_id)?.name} <span className="text-slate-400 font-medium ml-1">({p.payment_type})</span></p>
                                          <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-0.5">{new Date(p.payment_date).toLocaleDateString()} {p.notes && `• Ref: ${p.notes}`}</p>
                                        </div>
                                     </div>
                                     <div className="flex items-center gap-4">
                                       <p className="text-base font-bold text-emerald-600">₹{parseFloat(p.amount).toLocaleString()}</p>
                                       <button onClick={() => handleDeleteProjectPayment(p.id)} className="text-rose-400 hover:text-rose-600 bg-rose-50 p-1.5 rounded-lg transition-colors"><Trash2 className="h-4 w-4" /></button>
                                     </div>
                                  </div>
                                ))}
                             </div>
              ...
              <div className="p-6 border-t border-slate-100 bg-[#FAFCFF] flex justify-end items-center gap-4 shrink-0">
                ...
                <button onClick={() => setIsModalOpen(false)} className="rounded-xl border border-slate-200 bg-white h-12 px-8 font-bold text-sm text-slate-600 hover:bg-slate-50 shadow-sm transition-colors">Close Workspace</button>
                {(role === 'admin' || role === 'head') && modalTab === 'details' && (
                  <button onClick={handleSaveProject} disabled={isSaving} className="bg-gradient-to-r from-blue-900 to-indigo-800 text-white rounded-xl h-12 px-10 font-bold text-sm shadow-md shadow-blue-900/20 hover:shadow-lg hover:-translate-y-0.5 transition-all">
                    {isSaving ? "Saving..." : "Save Details & Auto-Invoice"}
                  </button>
                )}
              </div>
            </motion.div>
